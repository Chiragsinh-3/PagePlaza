...
const sendWelcomeEmail = async (to) => {
    ...
    const body = generateEmailTemplate("Welcome to PagePlaza", "Thank you for joining PagePlaza. We're excited to have you on board!", "Get Started", baseUrl, "If you have any questions, please contact us at support@pageplaza.com");
    await sendEmail(to, "Welcome to PagePlaza", body);
};
exports.sendWelcomeEmail = sendWelcomeEmail;
// Send verification email
const sendVerificationToEmail = async (to, token) => {
    const baseUrl = process.env.FRONTEND_URL || "http://localhost:8000";
    const verificationUrl = `${baseUrl}verify/${token}`;
    const body = generateEmailTemplate("Verify your PagePlaza account", "Click the button below to complete your registration", "Verify Email", verificationUrl, "If you didn't create this account, please ignore this email");
...
const sendPasswordResetEmail = async (to, token) => {
    ...
    const body = generateEmailTemplate("Reset your password", "We received a request to reset your PagePlaza password", "Reset Password", resetUrl, "If you didn't request this change, please ignore this email");
    await sendEmail(to, "Reset your password", body);
};
exports.sendPasswordResetEmail = sendPasswordResetEmail;
// Generate email template
const generateEmailTemplate = (title, description, buttonText, buttonUrl, footerText) => {
    return `
